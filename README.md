...
Rust 编写的 ChatGPT OAuth 账户网关。对外提供 Responses HTTP/SSE、Search JSON、独立 `sk-` Key 和管理员后台；按账户追踪请求、5h/7days 额度、Token 用量与人民币费用。
...
账户页的近 5h、近 7days 金额及周额度金额估算使用增量汇总：每个请求完成时，与请求终态在同一事务内按结束时间累计。重复提交终态不会重复计数，历史请求沿用完成时已记录的价格；打开账户页不重新扫描请求 JSON 或主动向上游采集额度。

统计区间为 `(开始时间, 结束时间]`。完整 UTC 小时读取汇总，首尾不足一小时的部分读取精简金额明细，保留时间边界精度；周额度估算仍排除 Spark 独立额度池和未调用上游的请求。普通金额窗口继续包含全部已结束、已归属该账户的请求，并单独统计未计价数量。

金额明细与对应小时汇总保留最近 8 天，每小时清理；缩短请求详情保留期不会丢掉这段期间的金额统计。总览使用的长期小时用量汇总保持原有保留策略。首次升级自动从仍存在的最近 8 天请求记录回填，已清理的历史明细无法补回。
...
- 每个入站请求默认最多调用一次上游。唯一例外：Responses 首次收到 HTTP 400 且 `error.code=invalid_encrypted_content`，并能移除 reasoning 条目中的 `encrypted_content` 时，在同一账户和绑定上补发一次，最多两次。保留摘要、非空 content、ID 和其他字段；清理后只剩 type 的空 reasoning 条目一并移除。压缩块和工具参数密文保持原样。HTTP 传输重试、重定向和授权失败后的生成重放仍关闭。OAuth 刷新和额度查询是独立控制请求。
- 加密推理恢复会丢弃被移除的推理状态，不能视为无损上下文迁移。已开始的 SSE、超时、网络错误、其他 HTTP 错误及无可清理内容均不触发；Search 和独立 Compact 不适用。补发前重查 Key、账户与模型状态，取消或停用后不换账户补发。请求详情显示触发原因、清理计数、上游尝试及最终结果，不保存密文或上游错误原文。
...
账户授权或导入成功后会自动尝试获取上游模型，也可从账户详情单独获取，或在「模型与价格」批量获取所有账户。使用各账户的 OAuth 请求 `/models?client_version=0.153.4`，仅保存模型名称、显示名、上下文长度和采集时间，不保存上游模型说明或提示词。批量任务在后台运行，界面展示进度和失败原因。
...
已覆盖迁移、并发等待、取消、身份换代、工具关联保留、加密上下文及其绑定 ID 透传、旧别名还原与不保存摘要、热超时、SSE 分帧、一般失败不重试及加密推理受限恢复、Token/Fast/图片计价、拒绝请求追踪和重启收尾。真实 ChatGPT OAuth、图片生成编辑、Codex/sub2api 长时间联调尚需使用专门测试账户验证；当前测试不读取本机 Codex 凭证，也不会向官方发起生成调用。
