...
impl AccessStore for PgStore {
    ...
    async fn delete_key(&self, id: Uuid) -> Result<()> {
        ...
        let changed = sqlx::query("UPDATE api_keys SET deleted_at=now(),enabled=false,group_id='00000000-0000-0000-0000-000000000001' WHERE id=$1 AND deleted_at IS NULL")
    ...
    async fn keys(&self) -> Result<Vec<GatewayKey>> {
        sqlx::query("SELECT id,group_id,name,prefix,enabled,created_at,last_used_at FROM api_keys WHERE deleted_at IS NULL ORDER BY created_at DESC").fetch_all(&self.pool).await.map_err(|_| Error::storage())?.iter().map(key).collect()
    }
    ...
    async fn key_by_hash(&self, hash: &str) -> Result<Option<GatewayKey>> {
        let row = sqlx::query("SELECT id,group_id,name,prefix,enabled,created_at,last_used_at FROM api_keys WHERE secret_hash=$1 AND enabled=TRUE AND deleted_at IS NULL").bind(hash).fetch_optional(&self.pool).await.map_err(|_| Error::storage())?;
        if let Some(r) = row {
            let k = key(&r)?;
    ...
    async fn set_key_enabled(&self, id: Uuid, enabled: bool) -> Result<GatewayKey> {
        let mut tx = self.pool.begin().await.map_err(|_| Error::storage())?;
        let row = sqlx::query(
            "UPDATE api_keys SET enabled=$2 WHERE id=$1 AND deleted_at IS NULL RETURNING *",
        )
        .bind(id)
        .bind(enabled)
    ...
    async fn update_key(&self, id: Uuid, name: &str, group_id: Uuid) -> Result<GatewayKey> {
        ...
        let previous: Uuid = sqlx::query_scalar(
            "SELECT group_id FROM api_keys WHERE id=$1 AND deleted_at IS NULL FOR UPDATE",
        )
        .bind(id)
        .fetch_optional(&mut *tx)
        .await
        .map_err(|_| Error::storage())?
        .ok_or_else(Error::not_found)?;
        let row = sqlx::query("UPDATE api_keys SET name=$2,group_id=$3 WHERE id=$1 AND deleted_at IS NULL RETURNING *")
            .bind(id)
            .bind(name)
            .bind(group_id)
            .fetch_one(&mut *tx)
