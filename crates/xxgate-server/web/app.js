...
function quota(windows,minutes){
 ...
 if(!entry||entry.stale)return '<span class="muted">'+(entry?'已过期':'未知')+'</span>';
 const w=entry.window||entry;
 return `<div class="quota" title="采集：${esc(dt(w.observed_at))}${w.resets_at?' · 重置：'+esc(dt(w.resets_at)):''}"><div class="quota-label"><span>${Number(w.used_percent).toFixed(1)}% 已用</span></div><meter min="0" max="100" low="80" high="95" optimum="0" value="${Math.min(100,w.used_percent)}" aria-label="${minutes===300?'5h':'7days'}额度已使用比例"></meter></div>`;
}
...
function weeklyEstimate(e,compact=false){
 ...
 return `<span class="weekly-estimate estimate-value" tabindex="0" title="${esc(hint)}" aria-label="预计周额度金额 ¥ ${money(e.total_cny)}；${esc(hint)}">${icon('trend')}<span><small>≈ ¥</small> ${compact?accountAmount(e.total_cny):money(e.total_cny)}</span></span><span class="sample-chip">${icon('usage')}<span>${Number(e.used_percent_delta).toFixed(1)}% ${e.sample_stale?'历史样本':'消耗样本'}</span></span>`;
}
async function accountsPage(api){
 ...
 const cards=visible.map(({account:a,quotas:q,spending:s,resets:z})=>{const r=d.runtime.accounts.find(x=>x.id===a.id)||{},models=accountModels(a);return `<article class="account-card" aria-label="账户 ${esc(a.name)}">
  <header class="account-card-head"><div class="account-identity"><button class="account-avatar" data-action="account-detail" data-id="${a.id}" aria-label="查看账户 ${esc(a.name)}">${icon('accounts')}</button><div><button class="ghost account-title" data-action="account-detail" data-id="${a.id}">${esc(a.name)}</button><span class="account-provider">${icon('shield')}ChatGPT OAuth${a.codex_only?'<span class="codex-only-label">仅 Codex</span>':''}</span><div class="account-group-tags">${groupChips(a.group_ids)}</div></div></div><div class="account-controls"><div class="account-state">${accountSwitch(a)}</div><button class="icon account-settings" data-action="account-edit" data-id="${a.id}" aria-label="设置账户 ${esc(a.name)}" title="账户设置">${icon('settings')}</button></div></header>
  <div class="account-data-grid">
   <section class="account-data account-load"><h3>并发</h3><div class="load-value" title="处理中 / 并发上限">${icon('activity')}<span><strong>${n(r.inflight||0)}</strong><small> / ${n(a.max_inflight)}</small></span></div><span class="queue-chip">${icon('queue')}<span>${n(r.queued||0)} 排队</span></span></section>
   <section class="account-data"><h3>账户额度</h3><div class="account-periods"><div class="quota-period"><span class="period-label">${icon('clock')}5h</span>${quota(q,300)}</div><div class="quota-period"><span class="period-label">${icon('calendar')}7days</span>${quota(q,10080)}</div></div></section>
   <section class="account-data"><h3>已计价金额</h3><div class="account-periods account-spend"><div><span class="period-label">近 5h</span>${spendingPeriod(s?.last_5h,true)}</div><div><span class="period-label">近 7days</span>${spendingPeriod(s?.last_7d,true)}</div></div></section>
   <section class="account-data account-estimate"><h3>周额度金额估算</h3>${weeklyEstimate(s?.weekly_estimate,true)}</section>
  </div>
  <div class="account-reset-summary"><span>${z?.pending?'重置结果待核实':z?.snapshot?'可用重置 '+n(z.usable_count)+' 次':'重置次数未查询'}</span><button class="small ghost" data-action="reset-open" data-id="${a.id}">${icon('refresh')}查询与重置</button></div><footer class="account-models-section"><div class="account-models-heading"><span class="account-count">${icon('models')}<span>${n(models.length)} 个模型</span></span><button class="small ghost model-edit" data-action="account-models-open" data-id="${a.id}">${icon('edit')}编辑模型</button></div><div class="account-models">${models.length?modelChips(models):'<span class="model-chip">未选择模型</span>'}</div></footer>
 </article>`;});
