...
function quota(windows,minutes){
 ...
 return `<div class="quota" title="采集：${esc(dt(w.observed_at))}"><div class="quota-label"><span>${Number(w.used_percent).toFixed(1)}% 已用${entry.stale?' · 待更新':''}</span></div><meter min="0" max="100" low="80" high="95" optimum="0" value="${Math.min(100,w.used_percent)}" aria-label="${AccountQuotas.windowLabel(minutes)}额度已使用比例"></meter><span class="quota-expiry">到期 ${w.resets_at?esc(dt(w.resets_at)):'未知'}</span></div>`;
}
...
function spendingPeriod(period,compact=false){if(!period||period.cny==null)return '<span class="muted" title="尚未确认当前额度周期，请采集额度">周期未知</span>';const hint=`本周期 ${dt(period?.starts_at)} 至 ${dt(period?.resets_at)} · 精确金额 ¥ ${money(period?.cny??'0')} · ${period?.requests??0} 次请求${period?.unpriced?'；其中 '+period.unpriced+' 次未完整计价，金额未计入':''}${period.history_complete===false?'；部分历史可能已清理，仅汇总仍保留的记录':''}`;return `<span class="amount-chip" tabindex="0" title="${esc(hint)}" aria-label="${esc(hint)}">${icon('wallet')}<span>¥ ${compact?accountAmount(period?.cny??'0'):money(period?.cny??'0')}</span>${period?.unpriced?'<sup class="partial-mark">*</sup>':''}${period.history_complete===false?'<small class="partial-history">部分历史</small>':''}</span>`;}
...
function cycleUsage(period){
 ...
 return `<div class="cycle-usage" title="${period?.history_complete===false?'部分历史可能已清理，仅汇总仍保留的记录':period?.missing_tokens?'部分请求缺少 Token 记录，仅汇总已知用量':'当前额度周期内已结束请求的用量'}"><span>请求 <b>${value(period?.requests)}</b> 次</span>${tokens('输入',period?.input_tokens)}${tokens('输出',period?.output_tokens)}${period?.missing_tokens?'<span class="partial-mark">*</span>':''}</div>`;
}
function quotaEstimate(e,compact=false,minutes=e?.window_minutes||10080){
 if(e?.total_cny==null)return `<span class="estimate-value estimate-empty" title="${e?.status==='insufficient_history'?'历史记录不足，暂不估算':'需要同一有效额度周期内至少 1% 的消耗变化和网关计价记录'}">${icon('trend')}<span>${e?.status==='insufficient_history'?'历史不足':'样本不足'}</span></span>`;
 ...
 return `<span class="weekly-estimate estimate-value" tabindex="0" title="${esc(hint)}" aria-label="预计 ${AccountQuotas.windowLabel(minutes)} 额度金额 ¥ ${money(e.total_cny)}；${esc(hint)}">${icon('trend')}<span><small>≈ ¥</small> ${compact?accountAmount(e.total_cny):money(e.total_cny)}</span></span><span class="sample-chip">${icon('usage')}<span>${Number(e.used_percent_delta).toFixed(1)}% 本周期 · ${n(e.requests)} 次${e.sample_stale?' · 待采集':''}</span></span>`;
}
async function accountsPage(api){
 ...
 const cards=visible.map(({account:a,email,plan_type,quotas:q,spending:s,resets:z})=>{const r=d.runtime.accounts.find(x=>x.id===a.id)||{},models=accountModels(a),periods=AccountQuotas.periods(plan_type,q,s),estimate=AccountQuotas.estimate(plan_type,q,s);return `<article class="account-row" aria-label="账户 ${esc(a.name)}">
  <div class="account-row-identity"><button class="ghost account-title" data-action="account-detail" data-id="${a.id}">${esc(a.name)}</button><span class="account-email" title="${esc(email||'邮箱未知')}">${esc(email||'邮箱未知')}</span><div class="account-row-meta"><span class="account-plan">${esc(AccountQuotas.planLabel(plan_type))}</span>${groupChips(a.group_ids)}${a.codex_only?'<span class="codex-only-label">仅 Codex</span>':''}<span title="处理中 / 并发上限">并发 ${n(r.inflight||0)}/${n(a.max_inflight)} · 排队 ${n(r.queued||0)}</span></div></div>
  <div class="account-row-periods">${periods.map(({minutes,label,period})=>`<section class="account-row-period"><div class="account-row-heading"><h3>${label}</h3>${spendingPeriod(period,true)}</div>${quota(q,minutes)}${cycleUsage(period)}</section>`).join('')||'<span class="muted">额度周期未知 · 请采集额度</span>'}</div>
  <section class="account-row-estimate"><h3>${estimate?esc(estimate.label)+' 额度金额估算':'额度金额估算'}</h3>${quotaEstimate(estimate?.value,true,estimate?.minutes)}</section>
...
async function modelsPage(api){
 ...
 return head('模型与价格','汇总所有账户的上游模型，填写标准价格；Fast 按倍率计算。',`<button data-action="model-edit">手动添加</button><button data-action="price-new" ${m.items.length?'':'disabled'}>快速填价</button><button class="primary" data-action="models-sync" ${j.running?'disabled':''}>${j.running?'获取中 '+j.completed+' / '+j.total:'从所有账户获取模型'}</button>`)+`<section class="search-price-bar"><div><h2>Search 按次计费</h2><p>所有账户与模型共用 · 成功请求计 1 次 · 不叠加 Token 费用或 Fast 倍率</p></div><div class="search-price-value">${sp.per_call==null?'未设置':'¥ '+money(sp.per_call)+' / 次'}</div><button data-action="search-price-edit">设置单价</button></section><div class="notice">${fetched} / ${d.accounts.length} 个账户已有上游目录 · 发现 ${d.items.length} 个模型${d.client_version?' · 获取版本 '+esc(d.client_version):''}${j.running?' · 正在获取，完成后自动更新列表':''}${failures.length?`<details><summary>${failures.length} 个账户最近获取失败，已保留之前的模型列表</summary>${failures.map(a=>`<p>${esc(a.name)}：${esc(a.error.message)}</p>`).join('')}</details>`:''}</div><section class="panel">${rows.length?table(['模型','状态','支持账户','输入 / 缓存 / 输出（¥ / 百万 Token）','Fast 倍率','操作'],rows):empty('从账户获取模型名称','先添加账户，再获取上游目录。无需逐个输入模型名称。','<button class="primary" data-action="models-sync">获取上游模型</button>')}</section><p class="muted">模型名称在账户间去重；标准价格与 Fast 倍率按模型保存，历史请求费用保持不变。</p>`;
}
...
function encryptedReasoningRecovery(d){
 ...
 return `<section class="detail-section encrypted-reasoning-recovery"><h3>加密上下文恢复</h3>${fact([['触发原因',esc((recovery.source==='sse_error'?'SSE 流内错误':'HTTP '+(recovery.status||400))+' · invalid_encrypted_content')],['恢复结果',esc(outcome)],['移除推理密文字段',n(c.encrypted_fields_removed)+' 项'],['替换不可用工具片段 / 涉及工具结果',n(c.encrypted_tool_parts_replaced)+' / '+n(c.tool_outputs_changed)],['移除空 content / 空 reasoning 条目',n(c.null_content_fields_removed)+' / '+n(c.empty_reasoning_items_removed)],['上游调用次数',n(r.upstream_attempts)]])}<p class="muted">在原账户和绑定上最多补发一次。保留摘要和可读工具结果；被拒绝的工具密文片段明确标为历史结果不可用，不推测工具是否执行成功。已交付模型事件后不再恢复。</p>${attempts.length?table(['上游尝试','HTTP 状态','响应头耗时','上游请求 ID'],attempts.map(a=>`<tr><td>第 ${n(a.attempt)} 次</td><td>${esc(a.status)}</td><td>${ms(a.headers_ms)}</td><td class="mono">${esc(a.upstream_request_id||'未提供')}</td></tr>`)):''}<p class="muted">未收到响应头的尝试请结合最终错误和请求时间线查看。</p></section>`;
}
